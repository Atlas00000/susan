...
interface BrandStoryBackgroundProps {
  className?: string
}

export function BrandStoryBackground({ className }: BrandStoryBackgroundProps) {
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className || ''}`}>
      ...
      {[...Array(18)].map((_, i) => (
        <motion.div
          key={i}
          className="absolute w-1.5 h-1.5 bg-luxury-gold/30 rounded-full"
          style={{
            left: `${8 + (i * 5)}%`,
            top: `${15 + (i * 4.5)}%`,
          }}
