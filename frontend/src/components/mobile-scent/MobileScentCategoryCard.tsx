...
interface MobileScentCategoryCardProps {
...
export function MobileScentCategoryCard({
  category,
  index,
  isSelected,
  onSelect,
  isInView,
}: MobileScentCategoryCardProps) {
  ...
  return (
    ...
    >
      <motion.div
        className="relative bg-luxury-charcoal/70 backdrop-blur-xl border border-luxury-gold/20 rounded-[2rem] p-6 overflow-hidden cursor-pointer"
        animate={{
      ...
      >
        ...
        >
          <div className={`absolute inset-0 bg-gradient-to-br ${category.color} rounded-[2rem] opacity-5`} />
        </motion.div>
        ...
        <motion.div
          ...
          animate={{
            x: isSelected || isHovered ? ['-100%', '200%'] : '-100%',
          }}
          transition={{
            duration: 1.5,
        ...
        <div className="relative z-10">
          <motion.h3
            className="text-xl md:text-2xl font-heading font-bold text-luxury-cream mb-3"
            animate={{
        ...
        {isSelected && (
          <motion.div
            className="absolute top-4 right-4 w-4 h-4 rounded-full bg-luxury-gold border-2 border-luxury-charcoal"
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
